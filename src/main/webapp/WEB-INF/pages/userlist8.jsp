...
<script type="text/javascript">
function toAdd(){
	window.location.href = ctx+"/user/toAdd8.html?selectType="+$("#selectType").val();
	return false;
}

 function toUpdate(id){
	window.location.href = ctx+"/user/toUpdate8.html?id="+id+"&selectType="+$("#selectType").val();
	return false;
}
...
function toDelete(id){
	$.ajax({
		...
		success:function(data){
			if("ok"==data){
				alert("删除成功");
				window.location.href=ctx+"/user/list8.html?selectType="+$("#selectType").val();
			}else{
				alert("删除失败");
			}
		},
...
<div class="bodywrapper">
    ...
    <form method="post" action="${pageContext.request.contextPath}/user/list8.html" id="sForm">
